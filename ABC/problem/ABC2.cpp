...
void init(int index)
{
    ...
    for(kk=1;kk<=S;kk++)
    {
        ...
        {          
                ...
                foods[index][num1]= Foods[index][kk][j].number;
                num1++;
...
void initial()
{
	...
	{
	   init(i);
	   /*fprintf(file,"foods[%d]=\n",i);
	   for(j=1;j<=J*S;j++)
	   fprintf(file,"%f ",foods[i][j]);
	   fprintf(file,"\n");*/
	}
...
int main()
{
    ...
    po s[200];
  	FILE *f1;
    file=fopen("result\\out19.txt","w");
	if((f1=fopen("input\\P10S5T05.dat","r"))==NULL)
	{
		exit(0);
	}
	fscanf(f1,"jobs %d\n",&J);
