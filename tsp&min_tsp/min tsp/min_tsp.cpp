...
#define  N	    51
#define  OPT	426
#define  it     5// the iteration times
...
int main()
{
	...
	/*for(i=0;i<5;i++)
	{
	   cpTO[i]=constructionPhase(N,a,cpSO[i],p2);
    }*/
    for(i=0;i<10;i++)
    {
          induce(N,cpSO[i]);
...
loopC1:	;
               ...
               double temp10;
               temp10=cpTO[j]+diss;
        	   fprintf(file,"temp10=%lf \n",temp10);
        	   fprintf(file,"cpSO[%d]=\n",j);
        	   for(k=0;k<N;k++)
        	   fprintf(file,"%d ",cpSO[j][k]);
        	   fprintf(file,"\n");
        	   
        	   cpTO[j]=evaldis(N,cpSO[j],p2);  
        	   fprintf(file,"cpTO[%d]=%lf \n",j,cpTO[j]);
