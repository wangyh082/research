/*void insertHeap(double *heap, int *heapi, int n, int s, double v, int vi)
...
/*void removeHeap(double *heap, int *heapi, int n, int s, double *v, int *vi)
...
// vi = index of the new element
{
	if (s > 0)
	{
		// return the first element of the heap which is the maximum one
		*v = heap[0];
		*vi = heapi[0];
		...
		while (maxChild < s-1 && heap[k] > heap[maxChild])
		{
			...
			exchange1(&heapi[k], &heapi[maxChild]);
			k = maxChild;
...
/*void sort(double *h, int *hi, int n)
...
{
	// heaps
	double *heap = (double *) malloc(n  *sizeof(int));
